# 爬取网易云音乐评论并把他们存入mysql or  本地文件
# 如果评论太多，会被封ip，这时需要加上代理ip
# 代码源自公众号日常学python  orancy 修改
...
class Spider():
    ...
    def __init__(self):
        ...
        self.url = 'https://music.163.com/weapi/v1/resource/comments/R_SO_4_2308499?csrf_token='
    #my heart will go on
    #https://music.163.com/weapi/v1/resource/comments/R_SO_4_2308499?csrf_token=
    ...
    def json2list(self,jsons):
        ...
        for user in users['comments']:
            ...
            time = user['time']
            user_dict = {'name': name, 'content': content, 'likedCount': likedCount,'userId': userId,'avatarUrl': avatarUrl,'commentId': commentId,'time': time}
            with open("我心永恒原始json.txt", 'a', encoding='utf-8')as f:
                str = json.dumps(user, ensure_ascii=False);
                f.writelines(str + "\n")
            #with open("多情content.txt", 'a', encoding='utf-8')as f:
...
class Operate_SQL():
    ...
    def __get_conn(self):
        try:
            ...
            self.conn = pymysql.connect(host='127.0.0.1',user='root',passwd='root',port=3306,db='music',charset="utf8mb4")
    ...
    def add_data(self,comment):
        '''增加一条数据到数据库'''
        # user_dict = {'name': name, 'content': content, 'likedCount': likedCount,'userId': userId,'avatarUrl': avatarUrl,'commentId': commentId,'time': time}
        sql = 'INSERT INTO `comments`(`name`,`content`,`likedCount`,`userId`,`avatarUrl`,`commentId`,`time`,`musicname`) VALUE(%s,%s,%s,%s,%s,%s,%s,%s)'
        try:
            self.__get_conn()
            cursor = self.conn.cursor()
            cursor.execute(sql, (comment['name'],comment['content'],comment['likedCount'],comment['userId'],comment['avatarUrl'],comment['commentId'],comment['time'],"多情"))
            self.conn.commit()
            return 1
...
class write_json():
    ...
    def write(self,comment):
        with open("我心永恒.txt", 'a', encoding='utf-8')as f:
            str = json.dumps(comment,ensure_ascii=False);
            print(str)
            f.writelines(str + "\n")
